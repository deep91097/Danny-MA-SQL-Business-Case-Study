...
VALUES
  ...
  --Q1
  select count(distinct node_id) as count_of_nodes from customer_nodes

  --Q2
  select r.region_name,count(cn.node_id) as number_of_nodes from customer_nodes cn left join regions r 
  on cn.region_id = r.region_id group by r.region_name
  go
  --Q3
  select r.region_name,count(distinct cn.customer_id) from customer_nodes cn left join regions r on r.region_id = cn.region_id
  group by r.region_name
  go
  --Q4
  with customer_details as (
  select customer_id,node_id,start_date,end_date,datediff(day,start_date,end_date) as diff from customer_nodes where end_date != '9999-12-31'
  group by customer_id,node_id,start_date,end_date 
  ),
  sum_diff as (
  select customer_id,node_id,sum(diff) as sum_of_days from customer_details group by customer_id,node_id
  )
  select AVG(sum_of_days) from sum_diff
  go
  --Q5

  -- PART 2 CUSTOMER TRANSACTIONS
  --Q1
  select txn_type,count(distinct txn_amount) as count_of_transactions,sum(txn_amount) as total_amount from customer_transactions
  group by txn_type
  go
  --Q2
  with customer_details as (
  select customer_id,txn_type,count(*) as txn_count,AVG(txn_amount) as avg_amount from customer_transactions group by customer_id,txn_type

  )
  select AVG(txn_count),AVG(avg_amount) from customer_details
  go
  --Q3
with customer_details as 
(
...
select month,count(customer_id) from customer_details where deposit_count >= 2 and (purchase_count >=1 or withdrawal_count >=1)
group by month
go
select 1
go
...
select customer_id,DATENAME(MONTH,txn_date) as txn_month,DATENAME(YEAR,txn_date) as txn_year,
sum(case when txn_type = 'deposit' then txn_amount else -txn_amount end) as net_monthly_txns
from customer_transactions group by customer_id,DATENAME(MONTH,txn_date),DATENAME(YEAR,txn_date) 
)
select customer_id,txn_month,txn_year,net_monthly_txns,sum(net_monthly_txns) over(partition by customer_id order by txn_month ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW)
as closing_balance from monthly_transaction_cte group by customer_id,txn_month,txn_year,net_monthly_txns order by customer_id
go
--Q5
with monthly_transaction_end_cte as (
select *,DATENAME(MONTH,txn_date) as txn_month,DATENAME(YEAR,txn_date) as txn_year,
LEAD(txn_date,1) over(partition by customer_id order by txn_date) as end_date from customer_transactions
)--,
--monthly_transactions_cte as 
--(
select customer_id,txn_month,txn_year,end_date,(select txn_date from monthly_transaction_end_cte where end_date is null),
sum(case when txn_type = 'deposit' then txn_amount else -txn_amount end) as net_monthly_txns
from monthly_transaction_end_cte group by customer_id,txn_month,txn_year,end_date
)
select customer_id,txn_month,txn_year,net_monthly_txns,sum(net_monthly_txns) over(partition by customer_id order by txn_month ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW)
as closing_balance from monthly_transactions_cte group by customer_id,txn_month,txn_year,net_monthly_txns order by customer_id
